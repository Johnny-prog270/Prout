...
def meshSol(L=2, H=1, order=1, smin=0.01, smax=0.1, dmin=0.1, dmax=0.3):
    ...
    # 1. Define Outer Boundary (Lines)
    p1 = gmsh.model.occ.addPoint(-1.0, -1.0, 0)
    p2 = gmsh.model.occ.addPoint(L, -1.0, 0)
    p3 = gmsh.model.occ.addPoint(L, H, 0)
    p4 = gmsh.model.occ.addPoint(-1.0, H, 0)
    ...
    # Airfoil Bezier Curves
    p_le = gmsh.model.occ.addPoint(0.0, 0.0, 0.0)
    p_top_ctrl1 = gmsh.model.occ.addPoint(0.0, 0.15, 0.0)
    p_top_ctrl2 = gmsh.model.occ.addPoint(0.4, 0.15, 0.0)
    p_te = gmsh.model.occ.addPoint(1.0, 0.0, 0.0)
    p_bot_ctrl1 = gmsh.model.occ.addPoint(0.4, -0.05, 0.0)
    p_bot_ctrl2 = gmsh.model.occ.addPoint(0.0, -0.05, 0.0)

    c_top = gmsh.model.occ.addBezier([p_le, p_top_ctrl1, p_top_ctrl2, p_te])
    c_bot = gmsh.model.occ.addBezier([p_te, p_bot_ctrl1, p_bot_ctrl2, p_le])

    airfoil_wire = gmsh.model.occ.addCurveLoop([c_top, c_bot])

    # 2. Generate a surface: first is external, next ones are cuts
    surface = gmsh.model.occ.addPlaneSurface([outer_wire, airfoil_wire])
    gmsh.model.occ.synchronize()

    # 3. Add Physical Groups : allows to retrieve bnd nodes after 
    gmsh.model.addPhysicalGroup(1, [l1, l2, l3, l4], tag=1)
    gmsh.model.setPhysicalName(1, 1, "OuterBoundary")

    gmsh.model.addPhysicalGroup(1, [c_top, c_bot], tag=2)
    gmsh.model.setPhysicalName(1, 2, "AirfoilBoundary")

    # 5. Size Fields

    # Field 1: Distance to the airfoil
    dist_field = gmsh.model.mesh.field.add("Distance")
    gmsh.model.mesh.field.setNumbers(dist_field, "CurvesList", [c_top, c_bot])

    # Field 2: Threshold based on airfoil distance
    threshold_field = gmsh.model.mesh.field.add("Threshold")
    gmsh.model.mesh.field.setNumber(threshold_field, "InField", dist_field)
    gmsh.model.mesh.field.setNumber(threshold_field, "SizeMin", smin)
    gmsh.model.mesh.field.setNumber(threshold_field, "SizeMax", smax)
    ...
    # Field 3: Conical wake behind the airfoil
    # We define a "V" shape using absolute values. 
    # Inside the cone |y| < 0.2*(x-1), we want smin. Outside, smax.
    cone_field = gmsh.model.mesh.field.add("MathEval")
    slope = 0.2  # Adjust this to make the cone wider or narrower
    x_start = 0.5
    # C is 1 inside the cone, 0 everywhere else
    # We use abs(y) to make it symmetric across the x-axis
    condition = f"Step(x - {x_start}) * Step({slope} * (x - {x_start}) - abs(y))"  
    expr = f"({condition}) * {smin*3} + (1 - ({condition})) * {smax}"

    gmsh.model.mesh.field.setString(cone_field, "F", expr)

    # Field 4: Minimum of both
    min_field = gmsh.model.mesh.field.add("Min")
    gmsh.model.mesh.field.setNumbers(min_field, "FieldsList", [threshold_field, cone_field])
    ...
    bnds_info = [("OuterBoundary", 1), ("AirfoilBoundary", 1)]
    bnds_tags = []
    
    # FIX: Changed `bnds` to `bnds_info`
    for name, dim in bnds_info:
        p_tag = next(g[1] for g in gmsh.model.getPhysicalGroups(dim) if gmsh.model.getPhysicalName(dim, g[1]) == name)
        bnds_tags.append(gmsh.model.mesh.getNodesForPhysicalGroup(dim, p_tag)[0])

    return elemType, nodeTags, nodeCoords, elemTags, elemNodeTags, bnds_info, bnds_tags

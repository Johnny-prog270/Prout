...
def assemble_stiffness_and_rhs_ex3(elemTags, conn, jac, det, xphys, w, N, gN, kappa_fun, rhs_fun):
    """
    Assemble global stiffness matrix and load vector for:
        -d/dx (kappa(x) du/dx) = f(x)
        ...
        for g in range(ngp):
            ...
            for a in range(nloc):
                ...
                for b in range(nloc):
                    ...
                    Nb = N[g, b]
                    gradNb = invjacg @ gN[g, b]
                    K[Ia, Ib] += wg *(kappa_g * float(np.dot(gradNa, gradNb)) + 1/kappa_g * Na * Nb)* detg
